...
// we would make an vector and there we need to store bi - ai. And now when we have got a vector which contains our extra_benefit as answer, we apply kadane algo to the vector to find largest sum contigous subarray. So, extra_benefit = maximum sum contigous subarray. And now we can return final_max_sum = original_array_sum + extra_benefit.
